  ...
  const onBack = () => {
    switch (post.state) {
      ...
      case POST_STATE.COMPLETE:
        return BottomTabNavigaton({navigation, tabName: BOTTOM_TAB_NAME.HISTORY});
      default:
  ...
  useEffect(() => {
    const listener = (data) => {
      if(data.isPayment || data.isPayment == "1"){
        Alert.alert("", "Khách hàng đã thanh toán VNPAY thành công!", [
          {
            text: "OK",
  ...
  const onChat = () => {
    ...
    Promise.all([
      ...
      .then(([box_chat_res, customer_res]) => {
        ...
        const avatar_url = customer_res.data.data.avatar_url;

        navigation.navigate("MessageScreen", {
          box_chat_id: box_chat_id,
          sender: sender,
          avatar_url: avatar_url,
          sender_id: post.customer.id,
    ...
    return (
      <View style={{ flexDirection: "row", justifyContent: "space-between", marginTop: 5 }}>
        ...
        {value == PAYMENT_METHOD.VNPAY ? (
          <Typography variant="TextBold">VNPAY</Typography>
        ) : (
          <Typography variant="TextBold">Tiền mặt</Typography>
        )}
      </View>
    );
  };
                ...
                <View style={modalStyle.wrapper}>
                  <View style={modalStyle.header}>
                    <Typography variant="H7">
                      Đánh giá về {post.customer.name}
                    </Typography>
                  </View>
  ...
  return (
    <>
      ...
      <SafeView>
        ...
        <DetailHeader navigation={navigation} title={"Chi tiết lịch hẹn"} onBack={onBack}/>
        <View style={style.default}>
          <ScrollView style={style.viewContent}>
            ...
            <View style={[style.viewItemContent2]}>
              ...
              <Typography style={{marginTop: 5}} variant="Text">
            ...
            <View style={style.btnGroup}>
              ...
              && post.payment_method == PAYMENT_METHOD.COD 
              && is_overdue
              && (
                <Button
              ...
              {post.state != POST_STATE.CANCEL &&
                post.state != POST_STATE.COMPLETE && (
                  <Button
                    style={style.cancelBtn}
                    size="modalBtn"
                    onPress={() => onCancel()}
